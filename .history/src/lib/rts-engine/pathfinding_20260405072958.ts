...
// Module-level cache: rebuilds only when the island set changes
let _adjCache: { key: string; adj: Map<string, string[]> } | null = null;

function buildAdj(islands: Island[]): Map<string, string[]> {
  const key = islands.map(i => i.id).join('|');
  if (_adjCache?.key === key) return _adjCache.adj;

  const adj = new Map<string, string[]>(islands.map(i => [i.id, []]));
  for (let i = 0; i < islands.length; i++) {
    for (let j = i + 1; j < islands.length; j++) {
      // Use <= 250 (was < 220) so islands exactly 220px apart are treated as connected
      if (rectGap(islands[i], islands[j]) <= 250) {
